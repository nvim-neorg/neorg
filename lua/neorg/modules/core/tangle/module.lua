                ...
                vim.uv.fs_open(file_str, "w", 438, function(werr, fd)
                    ...
                    if module.config.public.report_on_empty and write_content:len() == 0 then
                        vim.schedule(function()
                            utils.notify(
                                string.format("Tangled content for %s is empty.", file_str),
                                vim.log.levels.WARN
                            )
                        end)
                    end
                    ...
                    vim.uv.fs_close(fd, function(err)
                        assert(
                            not err,
                            lib.lazy_string_concat("Failed to close file '", file_str, "' for tangling: ", err)
                        )
                    end)
                end)
                ::continue::
