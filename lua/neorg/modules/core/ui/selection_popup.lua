...
return function(module)
    return {
        ...
        -- TODO: Rewrite all comments in this function
        create_multi_selection = function(name, config, callback)
            vim.validate({
                name = { name, "string" },
                config = { config, "table" },
                callback = { callback, "function" },
            ...
            -- Create a new horizontal split, we'll be placing our UI elements here
            local buf = module.public.create_split("multi_selection/" .. name, config.buffer_options)

            vim.notify("Press <Esc> to quit the window or <CR> to confirm")
            ...
            local location_in_flags, result = config.flags, {}
            local location_in_result = result
            ...
            while location_in_flags do
                ...
                if char == 27 then
                    ...
                    vim.api.nvim_buf_delete(buf, { force = true })
                    break
                elseif char == 13 then -- We have pressed <CR>
                    callback(result)
                    -- Delete the buffer and break out of the loop
                    vim.api.nvim_buf_delete(buf, { force = true })
                    break
                ...
                if data and not vim.tbl_isempty(data) and not data[3] then
                    ...
                    -- we've reached the end of our parsing, there's nothing more to traverse down:
                    if type(data[2]) == "string" then
                        if location_in_result[input] == nil then
                            location_in_result[input] = false
                        end

                        location_in_flags = config.flags
                        location_in_result[input] = not location_in_result[input]
                        location_in_result = result

                        display_values(name, location_in_flags)
                    else -- Otherwise we must be dealing with a table
                        if location_in_result[input] == nil then
                            location_in_result[input] = {}
                        end
                        ...
                        -- Recursively traverse down the table tree
                        location_in_flags = data[2].flags
                        location_in_result = location_in_result[input]
                    end
                end
            end
        end,
