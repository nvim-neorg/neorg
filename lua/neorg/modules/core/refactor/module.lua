...
module.public.rename_file = function(current_path, new_path)
    ...
    local current_file_changes = module.private.fix_links(current_path, function(link)
        ...
        if link_path and rel then
            ...
            local lp =
                Path(tostring(link_path)):relative_to(Path(new_path):parent(), true):resolve():remove_suffix(".norg")
            if lp then
...
module.public.rename_heading = function(line_number, new_heading)
...
module.private["refactor.rename.file"] = function(event)
...
module.private["refactor.rename.heading"] = function(event)
