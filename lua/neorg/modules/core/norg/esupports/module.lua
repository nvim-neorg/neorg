			...
			if type(data) == "table" and data.enabled then
				...
				if success then
					...
					if sub ~= vim.api.nvim_get_current_line() then
						...
						vim.api.nvim_win_set_cursor(0, { cursor_pos[1], cursor_pos[2] + (vim.api.nvim_strwidth(vim.api.nvim_get_current_line()) - vim.api.nvim_strwidth(line)) })
					end
...
module.on_event = function(event)
	...
	if event.type == "core.autocommands.events.textchangedi" and module.config.public.indent_config.realtime.enabled then
