    ...
    return neorg.lib.match(
      {
        data.type,
        ["task"] = function()
          return ((type(data.contexts) == "table" and not vim.tbl_isempty(data.contexts) or
            (type(data["waiting.for"]) == "table" and not vim.tbl_isempty(data["waiting.for"]))) and
            not data.inbox) or
            (type(data["time.due"]) == "table" and not vim.tbl_isempty(data["time.due"])) or
            (type(data["time.start"]) == "table" and not vim.tbl_isempty(data["time.start"]))
        end,
        ["project"] = function()
          if not tasks then
            return
          ...
          if
            type(data.contexts) == "table" and not vim.tbl_isempty(data.contexts) and
              vim.tbl_contains(data.contexts, "someday")
           then
            return true
          end
