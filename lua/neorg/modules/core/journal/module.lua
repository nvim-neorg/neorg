--[[
    ...
    ---
Journals are "periodic" in Neorg. You could have a daily journal, a weekly journal, a quarterly
journal, etc, (or all of them at once). Neorg ships with `daily`, `weekly`, and `monthly` journals
already defined, and `daily` is the default.
...
Some aliases exist for convenience (and backwards compatibility):
...
module.config.public = {
    ...
    -- ```lua
    -- daily = {
    --   start_date = os.time({ year = 2024, month = 6, day = 1, hour = 1 })
    ...
    for name, journal in pairs(module.config.public.journals) do
        ...
        if type(journal.path_format_strategy) == "function" then
            if not journal.parse_journal_path then
                log.error(
                    ("Journal `%s` has a `path_format_strategy` of type `function` but no `parse_journal_path` function was provided.")
                    :format(
                        name
                    )
                )
...
module.public = {
    ...
    open_journal = function(journal_name, date)
        ...
        local folder_name = module.config.private.journal_folder
        ...
        local journal_file_exists = dirman.file_exists(workspace_path / folder_name / path)

        dirman.create_file(folder_name / path, workspace)
...
module.private = {
    ...
    journal_current = function(args)
        local journal_name = args[1]
        if journal_name == nil then
            journal_name = module.config.public.default_journal_name
        end
        local current = module.private.get_period_date(journal_name)
    ...
    create_template = function(args)
        ...
        dirman.create_file(folder_name / template_name, workspace or dirman.get_current_workspace()[1])
    end,
    ...
    open_toc = function(args)
        ...
        if dirman.file_exists(folder_name / index) then
            dirman.open_file(workspace, folder_name / index)
        else
    ...
    create_toc = function(args)
        ...
        local journal_base_path = workspace_path / folder_name
        ...
        do
            ...
            -- Get the title from the metadata, else, default to the name of the file
...
local function handle_custom(args)
    ...
    if not args[2] then
        ...
        if not year or not month or not day then
            log.error("Must specify year month and day, in either YYYY-mm-dd or norg date format")
            return
        end
        local time = os.time({
            year = year,
